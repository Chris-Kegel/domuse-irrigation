...
const LOGO_SVG = `<svg viewBox="0 0 100 100"><rect width="100" height="100" rx="22" fill="#0E9CA5"/><path d="M50 12C32 42 24 54 24 66a26 26 0 1 0 52 0C76 54 68 42 50 12Z" fill="#fff"/><path d="M50 51L71 67 62 67 62 82 38 82 38 67 29 67Z" fill="#0E9CA5"/></svg>`;
...
class DomIrrigationPanel extends HTMLElement {
  ...
  _fmtDuration(s) {
    const sec = s.duration_seconds ?? ((s.duration_minutes ?? 0) * 60);
  ...
  async _addPump(name, entityId) {
    ...
    this._config.pumps.push(pump); this._addingPump = false; this._render();
  }
  async _removePump(id) {
    await this._hass.callWS({ type: `${DOMAIN}/remove_pump`, pump_id: id });
    this._config.pumps = this._config.pumps.filter(p => p.id !== id); this._render();
  }
  async _addSchedule(payload) {
    const s = await this._hass.callWS({ type: `${DOMAIN}/add_schedule`, ...payload });
    this._config.schedules.push(s); this._addingSchedule = false; this._newDays = []; this._render();
  }
  async _removeSchedule(id) {
    await this._hass.callWS({ type: `${DOMAIN}/remove_schedule`, schedule_id: id });
    this._config.schedules = this._config.schedules.filter(s => s.id !== id); this._render();
  }
  ...
  _updateLiveStates() {
    this.shadowRoot.querySelectorAll('[data-live-entity]').forEach(el => {
      const state = this._state(el.dataset.liveEntity), kind = el.dataset.liveKind;
  ...
  _htmlAddPumpForm() {
    return `<div class="form-card"><div class="form-title">Add Pump</div>
      ...
      <div class="field"><label>Switch Entity</label><select id="f-pump-entity"><option value="">Select a switch...</option>
  ...
  _htmlAddScheduleForm() {
    return `<div class="form-card"><div class="form-title">Add Schedule</div>
      ...
      <div class="field"><label>Pump</label><select id="f-sched-pump"><option value="">Select a pump...</option>
        ${this._config.pumps.map(p => `<option value="${this._esc(p.entity_id)}">${this._esc(p.name)}</option>`).join('')}
      </select></div>
      <div class="field"><label>Weekdays</label><div class="day-picker">
        ${DAYS.map((d, i) => `<button class="day-btn ${this._newDays.includes(i) ? 'sel' : ''}" data-action="toggle-day" data-day="${i}">${d}</button>`).join('')}
      </div></div>
      <div class="field"><label>Start Time</label><input id="f-sched-time" type="time" step="1" value="08:00:00"></div>
      <div class="field"><label>Duration (seconds)</label>
        <input id="f-sched-dur" type="number" value="600" min="1" max="86400" placeholder="e.g. 30=30s, 90=1m30s, 600=10m"></div>
      <div class="form-actions">
  ...
  _handleAction(el) {
    const { action, id, entityId, day } = el.dataset, sr = this.shadowRoot;
    switch (action) {
  ...
  _css() {
    return `<style>
      *,*::before,*::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .app-bar { display: flex; align-items: center; gap: 12px; padding: 14px 24px; background: var(--app-header-background-color,#1e293b); color: var(--app-header-text-color,#f1f5f9); font-size: 20px; font-weight: 600; box-shadow: 0 1px 0 rgba(255,255,255,.07); }
      .app-logo { width: 36px; height: 36px; flex-shrink: 0; }
      .app-logo svg { width: 36px; height: 36px; border-radius: 9px; display: block; }
      ...
      .tab.active { color: #0E9CA5; border-bottom-color: #0E9CA5; }
      ...
      .pump-icon.active { background: rgba(14,156,165,.2); }
      .pump-icon.active svg { fill: #0E9CA5; }
      .sched-icon { width: 40px; height: 40px; border-radius: 50%; flex-shrink: 0; background: rgba(14,156,165,.15); display: flex; align-items: center; justify-content: center; }
      .sched-icon svg { width: 22px; height: 22px; fill: #0E9CA5; }
      .badge { font-size: 10px; font-weight: 700; letter-spacing: .06em; padding: 3px 8px; border-radius: 99px; flex-shrink: 0; }
      .state-badge.on { background: rgba(14,156,165,.15); color: #0E9CA5; }
      ...
      .btn-primary { display: inline-flex; align-items: center; gap: 6px; padding: 9px 18px; border: none; border-radius: 8px; background: #0E9CA5; color: #fff; font-size: 13px; font-weight: 700; cursor: pointer; transition: opacity .15s; }
      ...
      .field input:focus, .field select:focus { border-color: #0E9CA5; }
      ...
      .day-chip { padding: 2px 7px; border-radius: 99px; background: rgba(14,156,165,.15); color: #0E9CA5; font-size: 10px; font-weight: 700; }
      ...
      .spinner { width: 36px; height: 36px; border-radius: 50%; border: 3px solid rgba(255,255,255,.1); border-top-color: #0E9CA5; animation: spin .7s linear infinite; margin-bottom: 14px; }
