/**
 * domuse-irrigation-panel
 * Sidebar panel for managing irrigation pumps and watering schedules.
 */
...
class DomIrrigationPanel extends HTMLElement {
  ...
  _updateLiveStates() {
    this.shadowRoot.querySelectorAll('[data-live-entity]').forEach(el => {
      const eid = el.dataset.liveEntity;
      ...
      <div class="panel">
        ...
        <div class="tabs">
          <button class="tab ${this._tab==='pumps'?'active':''}" data-tab="pumps">
            <svg viewBox="0 0 24 24"><path d="M20 10V8h-4V4h-2v4h-4V4H8v4H4v2h2v10h12V10h2zm-4 8h-2v-4h2v4zm-4 0h-2v-4h2v4z"/></svg>Pumps
          </button>
          <button class="tab ${this._tab==='schedules'?'active':''}" data-tab="schedules">
            <svg viewBox="0 0 24 24"><path d="M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5C3.89 3 3.01 3.9 3.01 5L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1h-2zm3 18H5V8h14v11z"/></svg>Schedules
          </button>
        </div>
        <div class="content">
          ${this._loading
  ...
  _htmlPumps() {
    ...
    return `
      ${this._addingPump ? this._htmlAddPumpForm() : `<button class="btn-primary add-btn" id="btn-add-pump"><svg viewBox="0 0 24 24"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>Add Pump</button>`}
      ${pumps.length === 0 && !this._addingPump ? this._htmlEmpty('No pumps yet. Add your first pump to get started.') : ''}
      ${pumps.length > 0 ? `<div class="card">${pumps.map(p => {
        ...
        return `<div class="list-row">
          ...
          <div class="list-info">
            ...
            <div class="list-sub">${this._esc(p.entity_id)}</div>
          </div>
          <span class="badge state-badge ${state}" data-live-entity="${this._esc(p.entity_id)}" data-live-kind="badge">${state.toUpperCase()}</span>
          <button class="toggle ${state==='on'?'on':''}" data-live-entity="${this._esc(p.entity_id)}" data-live-kind="toggle" data-state="${state}" data-action="toggle-pump" data-entity-id="${this._esc(p.entity_id)}"></button>
          <button class="icon-btn" data-action="remove-pump" data-id="${this._esc(p.id)}" title="Remove">
  ...
  _htmlAddPumpForm() {
    return `<div class="form-card">
      <div class="form-title">Add Pump</div>
      <div class="field"><label>Name</label><input id="f-pump-name" type="text" placeholder="e.g. Garden Zone 1"></div>
      <div class="field"><label>Switch Entity</label>
        <select id="f-pump-entity">
      ...
      <div class="form-actions">
        ...
        <button class="btn-primary" data-action="save-pump">Add Pump</button>
      </div>
    </div>`;
  }
  ...
  _htmlSchedules() {
    ...
    return `
      ${this._addingSchedule ? this._htmlAddScheduleForm() : `<button class="btn-primary add-btn" id="btn-add-schedule"><svg viewBox="0 0 24 24"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>Add Schedule</button>`}
      ${schedules.length === 0 && !this._addingSchedule ? this._htmlEmpty('No schedules yet. Create one to automate watering.') : ''}
      ${schedules.length > 0 ? `<div class="card">${schedules.map(s => {
        const pump = this._config.pumps.find(p => p.entity_id === s.pump_entity_id);
        return `<div class="list-row">
          <div class="sched-icon"><svg viewBox="0 0 24 24"><path d="M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5C3.89 3 3.01 3.9 3.01 5L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1h-2zm3 18H5V8h14v11z"/></svg></div>
          <div class="list-info">
            <div class="list-title">${this._esc(s.name||'Unnamed Schedule')}</div>
            <div class="list-sub">${this._esc(pump?pump.name:s.pump_entity_id)} · ${this._esc(s.time)} · ${s.duration_minutes} min</div>
            <div class="day-chips">${(s.weekdays??[]).map(d=>`<span class="day-chip">${DAYS[d]}</span>`).join('')}</div>
          </div>
          <button class="icon-btn" data-action="remove-schedule" data-id="${this._esc(s.id)}" title="Remove">
            <svg viewBox="0 0 24 24"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
          </button></div>`;
      }).join('')}</div>` : ''}`;
  }
  ...
  _htmlAddScheduleForm() {
    return `<div class="form-card">
      <div class="form-title">Add Schedule</div>
      <div class="field"><label>Name (optional)</label><input id="f-sched-name" type="text" placeholder="e.g. Morning Watering"></div>
      <div class="field"><label>Pump</label>
        <select id="f-sched-pump">
      ...
      <div class="field"><label>Weekdays</label>
        <div class="day-picker">
          ${DAYS.map((d,i)=>`<button class="day-btn ${this._newDays.includes(i)?'sel':''}" data-action="toggle-day" data-day="${i}">${d}</button>`).join('')}
        </div>
      </div>
      <div class="field"><label>Start Time</label><input id="f-sched-time" type="time" value="08:00"></div>
      <div class="field"><label>Duration (minutes)</label><input id="f-sched-dur" type="number" value="10" min="1" max="480"></div>
      <div class="form-actions">
        <button class="btn-secondary" data-action="cancel-schedule">Cancel</button>
        <button class="btn-primary" data-action="save-schedule">Add Schedule</button>
      </div>
    </div>`;
  }

  _htmlEmpty(msg) {
    return `<div class="empty"><svg viewBox="0 0 24 24"><path d="M12 2c-5.33 4.55-8 8.48-8 11.8C4 18.78 7.8 22 12 22s8-3.22 8-8.2C20 10.48 17.33 6.55 12 2z"/></svg><p>${msg}</p></div>`;
  }

  _bindEvents() {
    const sr = this.shadowRoot;
    sr.querySelectorAll('.tab').forEach(t => t.addEventListener('click', () => { this._tab = t.dataset.tab; this._render(); }));
    ...
    sr.querySelectorAll('[data-action]').forEach(el => el.addEventListener('click', () => this._handleAction(el)));
  }
  ...
  _handleAction(el) {
    ...
    const sr = this.shadowRoot;
    switch(action) {
      case 'cancel-pump':     this._addingPump = false; this._render(); break;
      case 'save-pump': {
      ...
      case 'remove-pump':     if (confirm('Remove this pump?')) this._removePump(id); break;
      case 'toggle-pump':     this._togglePump(entityId, el.dataset.state); break;
      case 'cancel-schedule': this._addingSchedule = false; this._newDays = []; this._render(); break;
      case 'save-schedule': {
        ...
        if (!pump || !time || !this._newDays.length) return;
        this._addSchedule({ name, pump_entity_id:pump, weekdays:[...this._newDays], time, duration_minutes:dur }); break;
      }
      case 'remove-schedule': if (confirm('Remove this schedule?')) this._removeSchedule(id); break;
      case 'toggle-day': {
        const d = parseInt(day, 10);
        this._newDays = this._newDays.includes(d) ? this._newDays.filter(x=>x!==d) : [...this._newDays,d].sort((a,b)=>a-b);
        el.classList.toggle('sel', this._newDays.includes(d)); break;
      }
    }
  }
