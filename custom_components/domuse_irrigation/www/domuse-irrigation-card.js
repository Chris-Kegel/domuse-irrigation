const IRRIGATION_DOMAIN = 'domuse_irrigation';
const CARD_LOGO = `<svg viewBox="0 0 100 100"><rect width="100" height="100" rx="22" fill="#0E9CA5"/><path d="M50 12C32 42 24 54 24 66a26 26 0 1 0 52 0C76 54 68 42 50 12Z" fill="#fff"/><path d="M50 51L71 67 62 67 62 82 38 82 38 67 29 67Z" fill="#0E9CA5"/></svg>`;
...
class DomIrrigationCard extends HTMLElement {
  ...
  async _loadPumps() {
    try {
      const data = await this._hass.callWS({ type: IRRIGATION_DOMAIN + '/get_config' });
  ...
  async _toggle(entityId, currentState) {
    try {
      await this._hass.callWS({
        type: IRRIGATION_DOMAIN + '/toggle_pump',
        entity_id: entityId,
  ...
  _updateStates() {
    ...
    (this._pumps ?? []).forEach(function(pump) {
      const state  = this._state(pump.entity_id);
      const eid    = CSS.escape(pump.entity_id);
      const toggle = sr.querySelector('.pump-toggle[data-eid="' + eid + '"]');
      const dot    = sr.querySelector('.state-dot[data-eid="' + eid + '"]');
      const badge  = sr.querySelector('.state-label[data-eid="' + eid + '"]');
      if (toggle) { toggle.classList.toggle('on', state === 'on'); toggle.dataset.state = state; }
      if (dot)    { dot.className = 'state-dot ' + state; }
      if (badge)  { badge.textContent = state.toUpperCase(); badge.className = 'state-label ' + state; }
    }.bind(this));
  }
  ...
  _render() {
    ...
    if (pumps === null) {
      ...
      body = '<div class="empty">No pumps configured.<br>Use the Irrigation panel to add pumps.</div>';
    } else {
      body = pumps.map(function(p) { return this._htmlPump(p); }.bind(this)).join('');
    }
    ...
    this.shadowRoot.innerHTML =
      ...
      '<ha-card>' +
        '<div class="card-header">' +
          '<div class="card-logo">' + CARD_LOGO + '</div>' +
    ...
    return (
      '<div class="pump-row">' +
        '<div class="pump-icon-wrap">' +
          '<div class="state-dot ' + state + '" data-eid="' + eid + '"></div>' +
        ...
        '<div class="pump-info">' +
          ...
          '<span class="state-label ' + state + '" data-eid="' + eid + '">' + state.toUpperCase() + '</span>' +
        '</div>' +
        '<button class="pump-toggle ' + (state === 'on' ? 'on' : '') + '"' +
                ' data-eid="' + eid + '" data-state="' + state + '"' +
                ' aria-label="Toggle ' + this._esc(pump.name) + '"></button>' +
      '</div>'
    );
  }

  _bindEvents() {
    this.shadowRoot.querySelectorAll('.pump-toggle').forEach(function(btn) {
      btn.addEventListener('click', function() { this._toggle(btn.dataset.eid, btn.dataset.state); }.bind(this));
    }.bind(this));
  }
  ...
  _css() {
    return '<style>' +
      ...
      '.card-header { display: flex; align-items: center; gap: 10px; padding: 14px 18px 10px; font-size: 17px; font-weight: 600; color: var(--primary-text-color); border-bottom: 1px solid var(--divider-color, rgba(0,0,0,.08)); }' +
      '.card-logo { width: 32px; height: 32px; flex-shrink: 0; }' +
      '.card-logo svg { width: 32px; height: 32px; border-radius: 8px; display: block; }' +
      ...
      '.spinner { width: 28px; height: 28px; border-radius: 50%; border: 3px solid rgba(0,0,0,.1); border-top-color: #0E9CA5; animation: spin .7s linear infinite; }' +
